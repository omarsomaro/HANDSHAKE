...
pub async fn handle_offer_generate(
    ...
    let (rendezvous, per_ephemeral_salt) = match req.passphrase {
        Some(pass) => {
            if stealth_mode {
                ...
                let (params, salt) = derive_from_passphrase_v2_stealth(&pass, &[0u8; 16]);
    ...
    if let Some(salt) = per_ephemeral_salt {
        ...
        // Recompute commit with salt included
...
fn discover_lan_endpoints(port: u16) -> anyhow::Result<Vec<Endpoint>> {
    let local_addrs = get_local_ip_addresses()?;
    let mut endpoints = Vec::new();
...
fn get_local_ip_addresses() -> anyhow::Result<Vec<IpAddr>> {

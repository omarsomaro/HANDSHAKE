...
pub async fn try_tor_connect(
    ...
    let isolation_key = isolation_key.map(|s| s.to_string())
    ...
    tracing::info!("Connecting via Tor SOCKS5 {} to {}:{}", socks_addr, onion_host, onion_port);
    ...
    let stream = if let Some(_iso_key) = isolation_key {
        ...
        match Socks5Stream::connect_with_password(
            ...
            Err(_) => {
                ...
                Socks5Stream::connect(socks_addr, (onion_host.as_str(), onion_port)).await?
            }
        }
    } else {
...
async fn configure_tor_bridge(socks_addr: &str, bridge: &TorBridge) -> Result<()> {
    ...
    let control_addr = socks_addr.rsplitn(2, ':').nth(1)
    ...
    let mut control_conn = TcpStream::connect(&control_addr).await
        .context("Tor control port not available")?;
...
pub async fn try_tor_listen(listen_addr: &str) -> Result<TcpListener> {
    let listener = TcpListener::bind(listen_addr)
        .await
        .context("Failed to bind Tor listener")?;
    
    let local_addr = listener.local_addr()?;
    tracing::info!("Tor Host: listening on {} (ensure Tor HiddenService forwards here)", local_addr);
...
mod tests {
    ...
    fn test_parse_onion_valid() {
        let (host, port) = parse_onion_addr(
            "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa.onion:9999"
        ).unwrap();
        assert_eq!(host, "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa.onion");
        assert_eq!(port, 9999);
    }

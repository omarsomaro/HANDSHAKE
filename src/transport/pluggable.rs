...
pub use mimicry::{ProtocolMimicry, create_protocol, list_protocols};
...
impl PluggableTransport {
    pub fn new(disguise: DpiDisguise, local: SocketAddr, remote: SocketAddr) -> Self {
        Self { disguise, local_addr: local, remote_addr: remote }
    }
    ...
    pub fn new_real_tls(cfg: &crate::config::Config, local: SocketAddr, remote: SocketAddr) -> Self {
        let domain = if let crate::config::PluggableTransportMode::RealTls(ref d) = cfg.pluggable_transport {
            d.clone()
        } else if !cfg.pluggable_tls_domains.is_empty() {
            use rand::seq::SliceRandom;
            cfg.pluggable_tls_domains.choose(&mut rand::thread_rng())
                .map(|s| s.clone())
    ...
    pub fn from_config(cfg: &crate::config::Config, local: SocketAddr, remote: SocketAddr) -> Self {
        match cfg.pluggable_transport {
            crate::config::PluggableTransportMode::None => Self::new(DpiDisguise::None, local, remote),
            crate::config::PluggableTransportMode::HttpsLike => Self::new(DpiDisguise::HttpsLike, local, remote),
            crate::config::PluggableTransportMode::FtpData => Self::new(DpiDisguise::FtpData, local, remote),
            crate::config::PluggableTransportMode::DnsTunnel => Self::new(DpiDisguise::DnsTunnel, local, remote),
            crate::config::PluggableTransportMode::RealTls(ref d) => Self::new(DpiDisguise::RealTls(d.clone()), local, remote),
...
impl HttpsLikeChannel {
    ...
    pub async fn perform_client_handshake(&mut self) -> Result<()> {
        ...
        if n < 100 { anyhow::bail!("Invalid TLS response"); }
        Ok(())
    }
    ...
    pub async fn perform_server_handshake(&mut self) -> Result<()> {
        ...
        if n < 100 { anyhow::bail!("Invalid TLS ClientHello"); }
...
impl FtpDataChannel {
    ...
    pub async fn perform_server_handshake(&mut self) -> Result<()> {
        ...
        let n = self.stream.read(&mut buf).await?;
        if &buf[..n] != b"PASV\r\n" {
            anyhow::bail!("Invalid FTP handshake");
        }
...
fn defragment_data(labels: &[&[u8]]) -> Result<Vec<u8>> {
	if labels.is_empty() {
		return Err(anyhow::anyhow!("No data labels"));
	}
	
	// Read header from first label
	if labels[0].len() < CHUNK_HEADER_SIZE {
		return Err(anyhow::anyhow!("Invalid label header"));
	}
	...
	for label in labels {
		...
		if chunk_total != total_chunks {
			return Err(anyhow::anyhow!("Mismatched chunk total"));
		}
	...
	for (i, chunk) in chunks.iter().enumerate() {
		match chunk {
			...
			None => return Err(anyhow::anyhow!("Missing chunk {}", i)),
		}
	}
...
impl DnsTunnelChannel {
    ...
    async fn cleanup_old_fragments(&mut self) {
        let now = Instant::now();
        self.recv_buffer.retain(|_, msg| {
            now.duration_since(msg.last_update) < Duration::from_secs(30)
        });
    }
    ...
    async fn store_fragment(&mut self, query_id: u16, labels: Vec<&[u8]>) -> Result<()> {
        ...
        let entry = self.recv_buffer.entry(query_id).or_insert_with(|| {
            FragmentedMessage::new(total_chunks)
        });
...
impl TransportChannel for DnsTunnelChannel {
	async fn read_message(&mut self) -> Result<Vec<u8>> {
		...
		let n = match tokio::time::timeout(Duration::from_secs(5), self.socket.recv(&mut buf)).await {
			Ok(result) => result?,
			Err(_) => anyhow::bail!("DNS response timeout"),
		};
		
		if n < 12 {
			anyhow::bail!("DNS packet too short ({} < 12)", n);
		}
		...
		if (flags & 0x8000) == 0 {
			anyhow::bail!("Not a DNS response (flags={:#06x})", flags);
		}
		if questions == 0 {
			anyhow::bail!("No questions in DNS response");
		}
		...
		while offset < buf.len() && buf[offset] != 0 {
			...
			if offset + label_len > buf.len() {
				anyhow::bail!("Invalid label length at offset {}", offset);
			}
		...
		if labels.is_empty() {
			anyhow::bail!("No labels found in DNS packet");
		}
	...
	match defragment_data(&labels) {
		...
		Err(e) => {
				...
				if e.to_string().contains("Missing chunk") {
					...
					anyhow::bail!("Partial DNS message, waiting for more chunks: {}", e);
				} else {
					Err(e)
				}
			}

...
pub async fn upnp_map(port: u16) -> Result<(IpAddr, u16)> {
    let (ext_ip, ext_port) = tokio::task::spawn_blocking(move || -> Result<(IpAddr, u16)> {
        use std::net::{UdpSocket as StdUdp, SocketAddr, SocketAddrV4, Ipv4Addr};
        ...
        let gateway = igd::search_gateway(Default::default())
            .context("no IGD gateway")?;
        let ext_ip = gateway.get_external_ip().context("get external ip failed")?;
...
pub async fn natpmp_map(port: u16) -> Result<(IpAddr, u16)> {
    ...
    n.send_public_address_request().await.context("natpmp public addr req")?;
    let public_ip = loop {
...
pub async fn pcp_map(port: u16) -> Result<(IpAddr, u16)> {
    ...
    socket.send_to(&request, pcp_server).await
    ...
    if version != PCP_VERSION {
        anyhow::bail!("PCP: version mismatch (got {}, expected {})", version, PCP_VERSION);
    }
    
    if opcode != PCP_OPCODE_MAP {
        anyhow::bail!("PCP: unexpected opcode (got {}, expected {})", opcode, PCP_OPCODE_MAP);
    }
    ...
    let external_ip = ipv4_from_mapped_ipv6(&buf[44..60])
        .context("PCP: external IP is not IPv4-mapped IPv6")?;
    
    tracing::info!("PCP mapping successful: {}:{}, from gateway {}", external_ip, external_port, from);
...
pub async fn init_wan_direct(port: u16, cfg: &crate::config::Config) -> Result<(UdpSocket, SocketAddr)> {
